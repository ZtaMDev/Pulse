...
### `._evaluate()`
Manually forces the guard to re-run its evaluator function. This is useful when the guard logic depends on external factors (like global variables or non-reactive APIs) that change.
